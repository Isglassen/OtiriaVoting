...
module.exports = new customClient_1.CommandData(new discord_js_1.SlashCommandBuilder()
    ...
    await interaction.client.database.pool.execute('DELETE FROM guilds WHERE guild_id = ? AND creation_time = ?', [args[0], args[1]]);
    await interaction.client.database.pool.execute('DELETE FROM votes WHERE guild_id = ? AND creation_time = ?', [args[0], args[1]]);
    await interaction.client.database.pool.execute('DELETE FROM choices WHERE guild_id = ? AND creation_time = ?', [args[0], args[1]]);
}, (0, idAutocorrect_1.default)(idAutocorrect_1.getAll));
